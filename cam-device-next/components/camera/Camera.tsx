...
import WebRTCStreamer from '../../libs/webrtcstreamer';
...
export const Camera = React.forwardRef<unknown, CameraProps>(
  (
    ...
    const [cameraName, setCameraName] = useState<string>(cameraDeviceName)
    ...
    useEffect(() => {
      const initCameraStream = async (
        ...
        if (streamServerUrl == null || streamServerUrl === '') {
          // If stream url is not present, use regular cam
          // constraints
          const constraints = {
            audio: false,
            video: {
          ...
          if (player.current != null) {
            // Check webRTC init
            const streamer:WebRTCStreamer | null = (window as any)["webRTCStreamer"]
            // Singleton?
            if (streamer == null) {
              const webrtcStreamer = new WebRTCStreamer(player.current, streamServerUrl);
              (window as any)["webRTCStreamer"] = webrtcStreamer
              let cName = cameraName
              if (cName.length <= 0) {
                const updateCameraName = await webrtcStreamer.getMediaList()
                setCameraName(updateCameraName[0])
                cName = updateCameraName[0]
              }
              try {
              ...
              streamer.updateMediaComponent(player.current)
              setNumberOfCameras(1)
            }
          } else {
            console.error("Player.current is null")
          }
        }
      };
