...
def read_image(imgpath:str, size:tuple[int, int] = (256, 256)) -> np.ndarray:
  ...
  if widthRatio > heightRatio:
    ...
    img = np.pad(img, ((0, 0), (padWidth, size[0] - padWidth - width), (0, 0)), 'constant', constant_values=0)
  else:
    # Fit to width
    ...
    img = np.pad(img, ((padHeight, size[0] - padHeight - height), (0, 0), (0, 0)), 'constant', constant_values=0)
...
def fit_image(interpreter:tf.lite.Interpreter, img:np.ndarray):
